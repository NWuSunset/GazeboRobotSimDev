...
ThrowSolution compute_solution(const InputParameters &inputs)
{
  ...
  : Node("ball_pail_planner")
  {
    ...
    input_subscription_ = create_subscription<std_msgs::msg::Float64MultiArray>(
      ...
      [this](const std_msgs::msg::Float64MultiArray::SharedPtr message) {
        ...
        publish_solution("subscription");
      });

    publish_solution("startup");
  }

private:
  void update_inputs(const std_msgs::msg::Float64MultiArray &message)
  {
  ...
  void publish_solution(const std::string &source)
  {
    ...
    const ThrowSolution solution = compute_solution(inputs_);
